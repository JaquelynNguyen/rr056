 ...
 */
@Autonomous(name = "CloseRed", group = "advanced")
@Disabled
public class CloseRed extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        Trajectory trajectory4R = drive.trajectoryBuilder(trajectory3R.end().plus(new Pose2d(0, 0, turnAngle1R)))
                .addTemporalMarker(0, () -> {
        ...
        Trajectory trajectory5R = drive.trajectoryBuilder(trajectory4R.end())
                ...
                .build();
//drop
        Trajectory trajectory6R = drive.trajectoryBuilder(trajectory5R.end())
                .back(3)
                .build();
...
//drop
        ...
        Trajectory trajectory3L = drive.trajectoryBuilder(trajectory2L.end())
                .back(7)
                .build();
        Trajectory trajectory4L = drive.trajectoryBuilder(trajectory3L.end())
                .strafeLeft(10)
                .build();
        ...
        Trajectory trajectory6L = drive.trajectoryBuilder(trajectory5L.end().plus(new Pose2d(0, 0, turnAngle1R)))
                .addTemporalMarker(0, () -> {
                    lift.setTarget(Lift.Presets.LOW_DROP);
                    arm.setPosition(Arm.Presets.OUTTAKE_POSITION);
                })
...
//drop
        Trajectory trajectory8L = drive.trajectoryBuilder(trajectory7L.end())
                .back(3)
                .build();
        Trajectory trajectory9L = drive.trajectoryBuilder(trajectory8L.end())
                .strafeRight(18)
                .build();
        Trajectory trajectory10L = drive.trajectoryBuilder(trajectory9L.end())
                .forward(10)
                .build();
        ...
        while (opModeIsActive() && !isStopRequested()) {
            ...
            switch (currentState) {
                ...
                case RETRACT_L:
                    ...
                    arm.setPosition(Arm.Presets.OUTTAKE_POSITION);
                    if (timer.seconds() > 1.5) {
                        currentState = State.TRAJECTORY_9L;
                        drive.followTrajectoryAsync(trajectory9L);
                    }
                    break;
                case TRAJECTORY_9L:
                    if (!drive.isBusy()) {
                        currentState = State.TRAJECTORY_10L;
                        drive.followTrajectoryAsync(trajectory10L);
                    }
                    break;
                case TRAJECTORY_10L:
                    if (!drive.isBusy()) {
